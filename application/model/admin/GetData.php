...
class GetData
{
	...
	{
		$query = 'SELECT list_category.* FROM list_category WHERE list_category.user_id = ? AND list_category.name LIKE ?';
	...
	{
		$query = 'SELECT list_exam.* FROM list_exam WHERE user_id = ? AND category_id = ? AND title LIKE ?';
